...
def extract_links_with_anchor(soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
    """
    提取链接及 anchor text
    """
    ...
    for a in soup.find_all("a", href=True):
        ...
        elif anchor_text and not dedup[u].get("anchor_text"):
            dedup[u]["anchor_text"] = anchor_text[:300]
    return [dedup[u] for u in sorted(dedup.keys())]

def html_to_text_and_links(html:str, base_url: str) -> Tuple[str, str, List[Dict[str, str]]]:
    soup = BeautifulSoup(html, "lxml")
    links = extract_links_with_anchor(soup, base_url=base_url)
...
def _contains_any(text:str, patterns: Iterable[str]) -> bool:
    text = (text or "").lower()
    return any(str(p).strip().lower() in text for p in patterns if str(p).strip())

def _default_deny_extensions() -> List[str]:
    return [
        ".pdf", ".png", ".jpg", ".jpeg", ".gif", ".svg", ".webp",
        ".zip", ".gz", ".tar", ".css", ".js", ".ico", ".mp4",
        ".mp3", ".xml", ".json", ".csv", ".xlsx", ".doc", ".docx",
        ".ppt", ".pptx", ".7z", ".rar", ".wasm", ".woff", ".woff2",
    ]

def html_crawl_reject_reason(url: str, seed_url: str, source: Dict[str, Any]) -> Optional[str]:
    u = normalize_url(url)
    if not u:
        return "invalid_url"
    ...
    # path_prefixes = as_list(source.get("path_prefix"))
    # if path_prefixes and not any(parsed.path.startswith(p) for p in path_prefixes):
        # return "outside_path_prefix"
    
    lowered_path = parsed.path.lower()
    deny_extensions = as_list(source.get("deny_extensions")) or _default_deny_extensions()
    deny_extensions = [
        x.lower() if x.startswith(".") else "." + x.lower()
        for x in deny_extensions
    ]
    if any(lowered_path.endswith(ext) for ext in deny_extensions):
        return "blocked_extension"
    
    full = u.lower()

    deny_patterns = as_list(source.get("deny_url_patterns"))
    if deny_extensions and _contains_any(full, deny_extensions):
        return "deny_url_pattern"
    
    # allow_patterns = as_list(source.get("allow_url_patterns"))
    # if allow_patterns and not _contains_any(full, allow_patterns):
    #     return "not_in_allow_url_patterns"

    return None

def html_allowed_for_crawl(url: str, seed_url: str, source: Dict[str, Any]) -> bool:
    return html_crawl_reject_reason(url, seed_url, source) is None

DEFAULT_LINK_SCORE_KEYWORDS = [
    "docs", "developers", "reference", "api", "protocol",
    "transaction", "transactions", "address", "wallet", "account",
    "token", "bridge", "smart-contract", "smart-contracts", "contract",
    "fee", "fees", "gas", "block", "blocks", "consensus", "staking",
    "privacy", "mixer", "risk", "compliance", "red-flag", "red-flags",
    "aml", "sanctions", "deposit", "withdraw", "transfer", "swap",
    "liquidity", "approval", "allowance", "mev",
]

DEFAULT_LINK_PENALTY_KEYWORDS = [
    "blog", "news", "careers", "jobs", "privacy-policy",
    "privacy_preferences", "terms", "cookie", "cookies", "login",
    "log-in", "signin", "sign-in", "signup", "sign-up", "contact",
    "about", "press", "media", "events", "community", "newsletter",
    "tag", "author", "search", "sitemap", "rss", "subscribe",
    "pricing", "partners", "brand",
]
...
def score_discovered_link(
    ...
    """
    对发现链接进行可解释性评分，返回值归一化到 [0, 1]
    """
    full = f"{url} {anchor_text or ''}".lower()
    reasons: List[str] = []
    score = 0.45

    priority = str(source.get("source_priority", "")).upper()
    if priority == "A":
        score += 0.08
        reasons.append("priority_A:+0.08")
    elif priority == "B":
        score += 0.03
        reasons.append("priority_B:+0.03")
    elif priority == "C":
        score -= 0.04
        reasons.append("priority_C:-0.04")

    authority = str(source.get("source_authority", "")).lower()
    if any(x in authority for x in ["official", "protocol", "developer", "docs", "standard"]):
        score += 0.05
        reasons.append("trusted_authority:+0.05")

    knowledge_domain = str(source.get("knowledge_domain", "")).lower()
    if knowledge_domain in {
        "basic_concept",
        "asset_profile",
        "chain_model",
        "transaction_type",
        ...
        score += 0.03
        reasons.append(f"knowledge_domain_{knowledge_domain}:+0.03")
    
    path_prefixes = as_list(source.get("path_prefix"))
    if path_prefixes:
        parsed_path = urlparse(url).path
        if any(parsed_path.startswith(p) for p in path_prefixes):
            score += 0.04
            reasons.append("inside_path_prefix:+0.04")
    
    positive_keywords = DEFAULT_LINK_SCORE_KEYWORDS + as_list(source.get("link_score_keywords"))
    matched_positive = []
    for kw in positive_keywords:
        if kw and kw.lower() in full:
            matched_positive.append(kw)
    
    if matched_positive:
        bouns = min(0.24, 0.04 * len(set(matched_positive)))
        score += bouns
        reasons.append(
            f"positive_keywords({','.join(sorted(set(matched_positive))[:8])}):+{bouns:.2f}"
        )
    
    negative_keywords = DEFAULT_LINK_PENALTY_KEYWORDS + as_list(source.get("link_penalty_keywords"))
    matched_negative = []
    ...
    if matched_negative:
        penalty = min(0.40, 0.08 * len(set(matched_negative)))
        score -= penalty
        reasons.append(
            f"penalty_keywords({','.join(sorted(set(matched_negative))[:8])}):-{penalty:.2f}"
        )

    depth_penalty = min(0.20, max(0, depth) * 0.05)
    if depth_penalty:
        score -= depth_penalty
        reasons.append(f"depth_{depth}:-{depth_penalty:.2f}")

    parsed = urlparse(url)
    if parsed.path in ("", "/") and depth > 0:
        score -= 0.10
        reasons.append("non_seed_root_page:-0.10")
...
def assess_content_quality(
    ...
    min_good_chars = int(source.get("min_good_content_chars", 300))

    if not content.strip():
        flags.append("empty_content")
        score -= 0.90
    elif char_count < min_good_chars:
    ...
    if any(x in text_l or x in html_l for x in js_markers) or (
        char_count < 200 and any(
            x in html_l
            for x in ['id="__next"', "id='__next'", 'id="root"', "id='root'", "window.__"]
        )
    ):
        flags.append("likely_js_shell")
        score -= 0.35
    ...
    if any(x in text_l for x in cookie_markers) and char_count < 1200:
        flags.append("likely_cookie_or_consent_page")
        score -= 0.30
    ...
    if any(x in text_l for x in access_markers):
        flags.append("likely_access_denied")
        score -= 0.55
    ...
    if len(lines) >= 20 and unique_line_ratio < 0.45:
        flags.append("low_unique_line_ratio")
        score -= 0.15
    
    if link_count >= 80 and char_count / max(1, link_count) < 60:
        flags.append("too_many_links_low_text")
        score -= 0.20
    ...
    if (
        ...
        flags.append("likely_navigation_page")
        score -= 0.25
    
    if any(x in text_l for x in ["sign in", "log in", "login"]):
        flags.append("likely_login_page")
        score -= 0.25
...
def infer_metadata_confidence(
    ...
    if link_score < 0.60 and depth > 0:
        confidence -= 0.10
    ...
    needs_review = bool(
        depth >= 2
        or (depth > 0 and link_score < 0.60)
        or (str(source.get("source_priority", "")).upper() == "C" and depth > 0)
        or confidence < 0.70
    )

    return confidence, needs_review

DISCOVERED_LINK_LOGS: List[Dict[str, Any]] = []
REJECTED_LINK_LOGS: List[Dict[str, Any]] = []


def reset_discovery_logs() -> None:
    DISCOVERED_LINK_LOGS.clear()
    REJECTED_LINK_LOGS.clear()


def log_discovered_link(item: Dict[str, Any]) -> None:
    DISCOVERED_LINK_LOGS.append(item)


def log_rejected_link(item: Dict[str, Any]) -> None:
    REJECTED_LINK_LOGS.append(item)
...
def save_outputs(
    ...
    # 可选发现日志：不替代 records.jsonl，只作为 html_crawl 质量审查辅助文件。
    discovered_links_path = out_dir / "discovered_links.jsonl"
    with discovered_links_path.open("w", encoding="utf-8") as f:
        for item in DISCOVERED_LINK_LOGS:
            f.write(json.dumps(item, ensure_ascii=False) + "\n")

    rejected_links_path = out_dir / "rejected_links.jsonl"
    with rejected_links_path.open("w", encoding="utf-8") as f:
        for item in REJECTED_LINK_LOGS:
            f.write(json.dumps(item, ensure_ascii=False) + "\n")
    ...
    discovery_depth_counter = Counter(
        str(r.get("discovery_depth", ""))
        for r in records
        if str(r.get("crawl_type", "")) == "html_crawl"
    )

    content_quality_flags_counter = Counter()
    for r in records:
        for flag in as_list(r.get("content_quality_flags")):
            content_quality_flags_counter[flag] += 1

    rejected_reason_counter = Counter(
        str(x.get("reason", ""))
        for x in REJECTED_LINK_LOGS
    )
    ...
    report = [
        ...
        f"discovered_links_jsonl: {discovered_links_path}",
        f"rejected_links_jsonl: {rejected_links_path}",
        "",
        "status_summary:",
    ]
    ...
    report.append("")
    report.append("html_crawl_discovery_summary:")
    report.append(f"- accepted_discovered_links: {len(DISCOVERED_LINK_LOGS)}")
    report.append(f"- rejected_links: {len(REJECTED_LINK_LOGS)}")

    report.append("- discovery_depth_records:")
    for k, v in discovery_depth_counter.most_common():
        report.append(f"  - depth={k}: {v}")

    report.append("- content_quality_flags:")
    for k, v in content_quality_flags_counter.most_common():
        report.append(f"  - {k}: {v}")

    report.append("- rejected_link_reasons:")
    for k, v in rejected_reason_counter.most_common():
        report.append(f"  - {k}: {v}")
...
def crawl_html_site(
    ...
    min_link_score = float(source.get("min_link_score", 0.55))
    min_content_quality_score = float(source.get("min_content_quality_score", 0.45))
    follow_low_quality_pages = bool(source.get("follow_low_quality_pages", False))

    print(
        f"[HTML_CRAWL] {source_id(source)} seed={seed_url}, "
        f"max_pages={max_pages}, max_depth={max_depth}, min_link_score={min_link_score}"
    )
    ...
    while queue and len(seen) < max_pages:
        ...
        link_score = float(item.get("link_score", 1.0 if depth == 0 else 0.0))
        link_score_reasons = list(item.get("link_score_reasons") or [])
        is_seed_url = bool(item.get("is_seed_url", False))

        if not url or url in seen:
            continue

        reject_reason = html_crawl_reject_reason(url, seed_url, source)
        if reject_reason:
            log_rejected_link(
                {
                    "source_id": source_id(source),
        ...
        print(
            f"  - [{len(seen)}/{max_pages}] "
            f"depth={depth} score={link_score:.2f} {url}"
        )
        ...
        base_extra = {
            ...
            "discovered_from": source_id(source),
            "parent_url": parent_url,
            "discovery_depth": depth,
            "discovery_anchor_text": anchor_text,
            "link_score": round(link_score, 4),
            "link_score_reasons": link_score_reasons,
            "metadata_inherited": True,
        }

        if data is None:
            metadata_confidence, needs_review = infer_metadata_confidence(
                url,
                source_name(source),
                source,
                depth,
                ...
                make_record(
                    ...
                    extra={
                        **base_extra,
                        ...
                        "content_quality_flags": ["fetch_failed"],
                        "metadata_confidence": metadata_confidence,
                        "needs_metadata_review": True if needs_review else depth > 0,
                        "link_count": 0,
        ...
        raw_path = dirs["raw_html"] / safe_name_from_url(
            url,
            ".html",
        ...
        content_quality_score, content_quality_flags = assess_content_quality(
            title,
            content,
            html,
            url,
        ...
        metadata_confidence, needs_metadata_review = infer_metadata_confidence(
            url,
            title,
            source,
            depth,
        ...
        text_path = dirs["clean_text"] / safe_name_from_url(
            url,
            ".txt",
            prefix=source_id(source),
        )
            ...
            make_record(
                ...
                extra={
                    **base_extra,
                    ...
                    # 保留旧字段 depth，新增字段使用 discovery_depth；
                    # 旧脚本即使读取 depth 也不受影响。
                    "depth": depth,
                    "link_count": len(links),

                    "content_quality_score": round(content_quality_score, 4),
        ...
        if content_quality_score < min_content_quality_score and not follow_low_quality_pages:
            # 低质量页面保留 record，但不继续扩展。
            continue

        candidates: List[Dict[str, Any]] = []

        for link_item in links:
            link_url = normalize_url(link_item.get("url"), url)
            link_anchor = str(link_item.get("anchor_text") or "")
            ...
            if link_url in seen or link_url in queued:
                continue

            reject_reason = html_crawl_reject_reason(link_url, seed_url, source)
            if reject_reason:
                log_rejected_link(
                    {
                        "source_id": source_id(source),
            ...
            candidate_score, candidate_reasons = score_discovered_link(
                link_url,
                link_anchor,
                source,
                depth + 1,
            ...
            if candidate_score < min_link_score:
                log_rejected_link(
                    {
                        "source_id": source_id(source),
                        "seed_url": seed_url,
                        ...
                        "discovery_depth": depth + 1,
                        "anchor_text": link_anchor,
                        "link_score": round(candidate_score, 4),
                        "link_score_reasons": candidate_reasons,
                        "reason": "link_score_below_min",
                        "crawl_time": now_str(),
                    }
                )
                continue
        ...
        # 仍是 BFS：只在同一层内按分数排序，让高价值链接更早进入 max_pages 窗口。
        candidates.sort(
            key=lambda x: float(x.get("link_score", 0.0)),
            reverse=True,
        )

        for candidate in candidates:
            # 防止极端目录页把队列撑得过大；不影响 max_pages 主约束。
